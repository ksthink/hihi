...
| 탭 | 핵심 기능 |
|---|---|
| **탐험** | 흑백 벡터 지도(등고선·음영기복·▲봉우리), 산 검색, 코스 선택(번호 배지·시종점), 산 소개, 날씨(기상청), 지도 다운 |
...
- `api/weather.js` = 기상청 프록시 — **키를 앱에 심지 않기 위해 웹·iOS 모두 이 경유를 유지**
...
├── weather.js · npn.js · poi-icons.js   도메인 로직 (iOS 직역 참조 / 관리자 공용)
...
- **날씨**: 기상청 단기예보 (data.go.kr) · **폰트**: MonaS12 (⚠️ 재배포 라이선스 확인 필요)
