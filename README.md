...
### 사용자 흐름
```
산 검색 → 지도·등산로 확인 → 지도 오프라인 저장 → 코스 선택 → 등반 시작(GPS 트래킹)
       → 기록 저장 → 루트 보기(정규 코스와 비교 · GPX 내보내기)
```
...
앱 단계**입니다(빌드 240번대, 실기 피드백 반복 중). 계획서의 검증 관문(스파이크) 상태:
...
| 탭 | 핵심 기능 |
|---|---|
| **탐험** | 흑백 벡터 지도, 큐레이션 코스(번호 배지 · 시종점 마커), 등고선, ▲봉우리 라벨, 산 검색, 산 소개 카드, 오늘 날씨(기상청), 위치 포인터 **헤딩 화살표** |
| **추천** | 운영자 큐레이션 매거진 캐러셀, 100대 명산 · BAC100 · 국립공원 아코디언 |
| **등반** | 코스 카드(거리·예상시간·난이도·고도 그래프), 저장된 지도 목록, 실시간 GPS 트래킹, 현재 위치 국가지점번호 |
| **기록** | 이메일 로그인(Supabase Auth), 산행 기록·통계·**산행 달력**, 프로필(닉네임·사진), 날짜 필터·정렬, **루트 보기**(RLS 로 본인만) |
...
- 위치 점 안쪽 도트가 천천히 깜박이는 **레코딩 표시** (탐험 포인터와 대비)

**위치 포인터 (iOS)**
- 점 둘레의 작은 화살표가 **폰이 향한 방향**을 실시간 표시 — 정북 고정 지도에서
  "지도상 내 시선"을 바로 읽음 (나침반 모드를 켜면 빔으로 전환)
- GPS 지터에 점이 떠다니지 않도록 **5m 거리 필터** (등반 트래킹과 같은 정책)

**루트 보기 (기록 탭 → "루트 ›", iOS)**
- 등반 UI 와 같은 **전용 집중 화면** — 날짜·거리·시간(시작~종료 시각)·누적고도
- **인터랙티브 고도 프로필** — 그래프를 누르면 그 지점이 지도 루트 위에 마커로 표시
- **정규 코스 비교** — 토글로 산의 정규 코스(원형 점 점선)를 걸었던 루트(굵은 실선) 위에 겹치고,
  코스대로 걸은 구간은 **색 반전**으로 한눈에 구분 (25m 근접 기하 계산)
- **GPX 내보내기** — 고도·시각 포함 표준 GPX 1.1, iOS 공유 시트로 저장·전송
  (파일명 `아이디_산코드_날짜.gpx`)
...
- **고도 데이터** — Copernicus GLO-30 DEM(표면모델·DSM)을 산별로 캐시해 등고선·고도 프로필을
  계산합니다. GPX 업로드 코스는 **GPX 자체 고도(`<ele>`)가 있으면 그것을 우선**(다리·건물의
  DSM 스파이크 회피), 없을 때만 DEM 을 25m 간격 표본 + 3m 임계 히스테리시스로 적산합니다.
...
- 스타일 JSON 은 빌드 산출물입니다 — `gen-style.mjs` 가 매 빌드 때 R2 의 관리자 표시 설정을
  구워 넣으므로, **표시 설정을 바꾸면 재빌드해야 앱에 반영**됩니다.
...
화면은 상단 **[산 편집] [큐레이션]** 2탭 — 산 편집 안에 산 목록/등산로/스팟 서브탭,
스팟·기저 POI 표시 설정은 지도 위 오버레이 카드에서 미리 보며 조정합니다.
...
- **목록에 없는 산 직접 추가** — 산림청 원본이 없는 도심·소규모 산(예: 목동 용왕산)은
  검색창 아래 **"＋ 목록에 없는 산 직접 추가"** 로 이름 + 지도 클릭 중심좌표만으로 등록합니다.
  - 산코드는 `9`로 시작하는 예약 대역(`900000001`~) — 산림청 코드(1~4 시작)와 영구 무충돌
  - 구간망이 없으므로 코스는 **GPX 원본 그대로**(스냅 없이) 등록됩니다. 끊긴 트랙은
    trkseg 경계대로 파트 분리, 고도는 GPX `<ele>` 우선
  - 배포 산출물이 여느 산과 동일해 앱 코드 변경 없이 그대로 소비
- **앱에서 삭제**: 편집 패널의 "앱에서 삭제" — Supabase 카탈로그 행 + R2 팩 파일 + 로컬 초안을
  한 번에 제거. 일시적으로 감추려면 "공개" 해제 후 배포.

> **산 식별자 = 산림청 산코드 9자리**(수동 등록은 `9xxxxxxxx`). 이 코드가 R2 폴더명·DB·저장 팩
> 키에서 전부 공유되므로 한 곳이라도 다르면 연결이 끊깁니다.
>
...
- 글리프 URL 은 런타임에 번들 절대 `file://` 로 재작성합니다 — 상대경로는 MapLibre Native 가
  resolve 하지 못해 **모든 지도 라벨이 통째로 사라집니다** (겪음 · [ISSUE.md](ISSUE.md) 참고).
...
├── poi-icons.js          기저지도 POI 아이콘 생성기 (웹앱·관리자 공용)
...
│   └── HiHeight/             Swift 소스 35개 (+ 개발자 모드 DevMode/)
│       ├── MapView.swift         MapLibre 래핑 (오프라인 전환·등반/기록 레이어·헤딩 화살표)
│       ├── ExploreView.swift     탐험 탭 (바텀시트·등반 HUD·루트 보기)
│       ├── RouteViewUI.swift     루트 보기 (고도 프로필·GPX 내보내기·코스 겹침 계산)
...
│   ├── pack_lib.py           공용 라이브러리 (좌표변환·그래프·DEM·고도 프로파일)
│   ├── draft_store.py        산별 큐레이션 초안 저장소 (+ 수동 산 등록)
│   ├── gpx_match.py          GPX → 구간망 맵매칭 · 원본 패스스루(수동 산)
...
│
└── 문서: README.md · WORD.md · IOS.md · CLOUDFLARE.md · WORKLOG.md · ISSUE.md · QA.md · CLAUDE.md
```
...
| `config/*.json` | Cloudflare R2 | 큐레이션 · 스팟/POI 표시 정책 (`no-cache`) |
...
- 한국 정밀 국가기본도의 국외 반출 규제를 고려해, 지도 데이터 호스팅·고도 계산은
  국내(서울) 리전 서버에서 수행하고 앱에는 파생 수치만 담습니다.
...
| [ios/DEVMODE.md](ios/DEVMODE.md) | 개발자 모드 HUD — 사용법과 **완전 삭제** 절차 |
