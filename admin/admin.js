...
$("draft-del").onclick = async () => {
  if (!S.code) return;
  if (!confirm(`${S.draft.mountain.name} 초안(admin_data/${S.code})을 제거할까요?\nStorage/DB 배포본은 유지됩니다.`)) return;
  S.dirty = false; // 제거할 초안을 다시 저장하지 않도록
  await api(`/mountains/${S.code}`, { method: "DELETE" });
