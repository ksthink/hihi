...
> 재편되어 이식 범위·전제가 달라졌다.
>
> **⚠️ 2026-09-09 갱신 — 이식은 완료됐다. 지금 무엇이 남았는지는 §0 만 보면 된다.**
> §1~§13 은 2026-07-13 시점의 계획으로 그대로 보존한다(판단의 근거를 남기기 위해).
> 계약 변경 규칙(§11)과 규제·심사(§12)는 여전히 유효하다.

---

## 0. 현황 (2026-09-09 갱신) — **이식 완료. 이 문서는 계획서에서 이력으로 성격이 바뀌었다.**

> 아래 §1~§13 은 **2026-07-13 시점의 계획**이며 그대로 보존한다(왜 그렇게 정했는지의 기록).
> 실제 진행은 계획을 앞질렀으므로, **지금 무엇이 남았는지는 이 절만 보면 된다.**

**실측 기준**: `ios/HiHeight/` **45개 파일 · 7,664줄**. 웹 사용자 앱(`app.js`·`weather.js`·
`index.html`·`style.css` 합계 3,773줄)의 **약 2배**다. 앱은 더 이상 웹의 이식본이 아니라 본체다.
실기기(iPhone 12 mini · 16 Pro) 등반 기록이 **빌드 329**까지 쌓여 있다.

### 완료

- **§9 스파이크 S1·S2·S3** — go/no-go 관문. 실기기 백그라운드 등반 기록(`fg_s`/`bg_s`/`lpm` 포함)이
  누적돼 있으므로 셋 다 통과한 지 오래다. **이 절은 더 이상 의사결정에 쓰이지 않는다.**
- **§6 기능 인벤토리 4탭 전부** — 탐험(카탈로그·검색·코스·스팟·산 소개·날씨·국가지점번호·지도 다운),
  추천(캐러셀·아카이브·공식 추천 3종), 등반(저장된 지도·시작/종료·라이브 루트·기록 저장),
  기록(목록·루트 보기·스와이프 삭제·산행 달력·합계·계정). **누락 없음.**
- **§7-1~7-5** — 백그라운드 GPS(`ClimbStore`), 원본 GPX 로컬 보관·공유시트(`ExploreView`
  `ShareSheet`), 팩 파일시스템·버전 비교(`PackStore`), `fg_s`/`bg_s`/`lpm` 계측, 배터리 모드 3단(`BatteryMode`).
- **§8-1 R2 커스텀 도메인** — `hihi.metaphr.dev`(팩·config·이미지) + `hihi.ksthink.com`(날씨 프록시), `Config.swift`.
- **§8-3 얼룩 폴리곤** — 2026-07-22 해결(12일). 경위는 `ISSUE.md` #1.
- **§8-5 Apple Developer** — TestFlight 운용 중.

### 계획에 없던 것 — 앱이 계획을 넘어선 부분

계획서 §6 인벤토리에 **존재하지 않는** 기능들이다. 이 문서가 낡은 가장 큰 이유:

| 기능 | 파일 |
|---|---|
| 대기질/미세먼지 — 최근접 측정소 실황·예보·지도 핀·시트 | `AirQuality` · `AirStationSheet` · `AirIcon` |
| 내비 화살표 + 방위 고리 | `NavView`(308줄) · `CourseNav` |
| 일출·일몰 | `SunTimes` |
| 등반 재개 · 짧은 등반 확인 · 종료 확인 | `ClimbResumeView` · `ShortClimbConfirmView` · `ClimbEndConfirmView` |
| 아바타 크롭 · GIF 커버 재생 · 스와이프 삭제(UIKit 팬 재구현) | `AvatarCropView` · `GifView` · `SwipeToDeleteRow` |

### 남은 일 (앱)

1. **⚠️ §8-4 출처표기 누락 — 최우선.** 현재 앱·웹 모두 `Protomaps · OpenStreetMap · Copernicus DEM`
   까지만 표기한다. **산림청(등산로·100대 명산 API) · 국립공원공단 · 기상청 표기가 어디에도 없다**
   (`grep` 전수 확인, 2026-09-09). §12 의 저작자 표시 의무와 직결되고 심사 항목이기도 하다.
   웹·앱 양쪽에 필요.
2. **§7-6 HealthKit · 위젯 · 라이브 액티비티** — 원래 "후순위·선택". **할지 말지 결정만 남았다.**
3. **팩 용량 표시(iOS 누락)** — 지도 다운로드 확인 창(`PackPromptView`)에 용량이 없다.
   웹은 `app.js` 가 `mountains.pack_size_kb` 로 "약 2.9MB" 를 보여준다. Swift 에 `pack_size_kb`
   참조가 **0건**. 값은 카탈로그에 이미 있으므로 표시만 붙이면 된다.

### 맥북 백로그 (EC2 작업에서 파생 — 맥북에서만 갚을 수 있다)

EC2 에서 관리자·데이터를 고칠 때마다 앱 쪽 대응이 생긴다. 발견 즉시 여기 모은다.
...
이식 계획으로서의 역할은 끝났다. §1~§13 은 이력으로 두고, **앞으로의 앱 작업은 이 §0 의
"남은 일"·"맥북 백로그"에 누적**한다. 계약(팩 규격·config·트랙 포맷) 변경 규칙(§11)과
규제·심사(§12)는 여전히 유효하다.
...
> **상태(2026-09-09): 아래 4탭 전 항목 이식 완료.** 이 표에 **없는** 기능이 그 뒤로 여럿 추가됐다(대기질·내비 방위 고리·일출일몰 등) — §0 참조.
...
> **상태(2026-09-09): 1~5 완료 · 6 미착수(선택).** 상세는 §0.
...
> **상태(2026-09-09): 1·2·3·5 완료 · 4 출처표기 미완(⚠️ 최우선 잔여) · 6 웹 동결은 지켜지지 않음.** 상세는 §0.
...
> **상태(2026-09-09): S1·S2·S3 모두 통과했고 관문은 닫혔다.** 실기기 빌드 329 까지 운용 중이므로 이 절은 이력이다.
...
> **상태(2026-09-09): M1~M5 완료. M6 은 출처표기만 남았다**(§0 남은 일 1번).
