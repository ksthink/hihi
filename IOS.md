...
4. **등반 진단 계측 보강 (배터리 분석용)** — `track.meta` 에 **전경/배경 시간(`fg_s`/`bg_s`,
   scenePhase 누적)**과 **저전력모드(`lpm`, `ProcessInfo.isLowPowerModeEnabled` — 세션 중 1회라도
   켜졌으면 true)** 추가. 배경: 2026-07-31 분석에서 유일한 유효 측정치(Best 1Hz, 27분)가
   11.1%/h 로 예산(4h ≤25% = 6.25%/h)의 1.8배였으나, 화면 기여분과 GPS 기여분을 분리할 수
   없었고 잔량 15% 구간은 저전력모드 개입 여부도 알 수 없었다. 관리자 등반 기록 표·xlsx 에
   두 열 노출(웹 admin.js 는 meta 키만 추가하면 됨). `supabase/schema.sql` meta 주석 갱신 포함.
5. **등반 배터리 모드 3단 (일반/절전/최대절전)** — 배경: 네비 모드(화면 켜고 1Hz 지도 갱신)
   실측 42~65%/h (2026-08-01). 렌더·화면 비용을 단계별로 끄는 사용자 선택:
   - **일반** — 현행 1Hz 연속 네비.
   - **절전** — 지도 유지, 화면 갱신만 **이동 5m·최장 5초** 스로틀(정지 시 갱신 0,
     갱신 시 0.3초 이동 애니메이션 후 idle, 30° 이상 방향 전환 시 즉시 1회). 렌더 ~80% 절감.
   - **최대절전** — 지도 미표시: 검정 바탕 + 큰 숫자(경과·거리·누적상승·고도)만, 탭하면
     지도 일시 복귀. 렌더 100% + OLED 대부분 절감.
   - **설정 위치: 기록 탭 → 프로필 → 설정** (ProfileEditView 계열). UserDefaults 저장,
     등반 시작 시 적용. UI 에 초 단위 노출 금지 — 라벨+한 줄 설명, **"어느 모드든 GPS
     기록(1Hz)은 동일" 문구 필수**(스로틀은 표시 전용, 기록·통계 무손실).
   - `track.meta` 에 선택 모드 기록(§7-4 계측과 세트 — 모드별 %/h 를 관리자 표로 검증).
   - (선택) 배터리 30% 미만이면 절전 권장 넛지.
6. **(후순위·선택)** HealthKit 운동 연동, 라이브 액티비티(등반 중 잠금화면 통계), 위젯.
