...
### 생성 / 추가
- **끌어올리기 스크롤 간헐 막힘 + 바텀시트 그래버 끊김 수정(`SwipeToDeleteRow`·`ExploreView`)**: ① 기록·등반 탭에서 아래→위 스크롤이 간헐적으로 안 되던 원인 = 각 행의 `SwipeToDeleteRow` 가 `.gesture(DragGesture)` 로 세로 드래그까지 선점한 뒤 가로 우세 가드로 무시 → 그 터치가 List 스크롤로 넘어가지 못함. `.gesture` → `.simultaneousGesture` 로 바꿔 스크롤과 동시 인식(가로일 때만 카드 이동, 세로 스크롤 항상 유지). ② 그래버를 SF Symbol `Image` 로 바꿨더니 시트가 매 프레임 재구성될 때 무거워 끊겨 보임 → 가벼운 Path 도형 꺾쇠(`Chevron`, animatableData 로 ^⇄⌄ 모프)로 교체.
