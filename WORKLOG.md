...
### 수정 / 변경
- **화살표를 픽셀아트로(`NavView.PixelArrow`)**: SF Symbol 은 곡선·베벨이 있어 매끈한 인상인데 이 앱의 흑백·저해상 톤에는 계단 모서리가 맞다. 11×11 격자 문자열 패턴을 `Canvas` 로 찍는다(모양 수정 = `#`/`.` 편집). 인접 칸을 **0.5pt 씩 겹쳐** 안티앨리어싱 실선이 경계에 비치는 것을 막았다.
