...
### 후속 (이 세션에서 하지 않음)
- **가리왕산제1코스가 `ready` 로 바뀌었다** — 운영자가 관리자에서 시점↔종점을 뒤집고(저장 좌표가
  보존 원본 `c16c9b185946.gpx` 의 정확한 역순 = 들머리→정상, 누적상승 68m→1169m) 공개 전환했다.
  ⚠️ **재발행해야 팩에 들어간다**(통합배포 당시엔 `draft` 라 제외됐다).
- 커밋 `b52f56d`·`bf0246e` 는 **로컬에만 있다**(푸시 전).
